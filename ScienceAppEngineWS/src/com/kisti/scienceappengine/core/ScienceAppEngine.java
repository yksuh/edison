package com.kisti.scienceappengine.core;
...
import com.kisti.scienceappengine.dbaccess.ForeignKey;
import com.kisti.scienceappengine.dbaccess.GeneralDBMS;
import com.kisti.scienceappengine.dbaccess.InternalTable;
import com.kisti.scienceappengine.dbaccess.MySQLSubject;
...
class Library {
	...
	public String toStringInXML() {
		...
		/***
		 * Any of these elements shouldn't be null. Otherwise, we should report
		 * an error message. But for simplicity we'll take care of such a case
		 * as empty element.
		 */
		...
		if (this.linuxCLibVer != "") {
			sb.append("\t\t<libCLibVer>" + this.linuxCLibVer
					+ "</libCLibVer>\n");
		}
		// Let's make an empty closed element.
		else {
...
public class ScienceAppEngine {
	...
	public static final InternalTable SOLVER = new InternalTable(
			Constants.TABLE_PREFIX + Constants.TABLE_SOLVER, new String[] {
					"SolverID", // solver id
					"Title", // solver title
					"Name", // solver name
	...
	public static final InternalTable SOLVER_TEST = new InternalTable(
			Constants.TABLE_PREFIX + Constants.TABLE_SOLVER_TEST, new String[] {
			...
			new ForeignKey[] { new ForeignKey(new String[] { "SolverID" },
					Constants.TABLE_PREFIX + Constants.TABLE_SOLVER,
					new String[] { "SolverID" }, " ON DELETE CASCADE") }, // foreign
																			// key
			"" // sequence name
	);
	...
	public static final InternalTable SOLVER_TEST_LOG = new InternalTable(
			...
			new ForeignKey[] { new ForeignKey(new String[] { "TestID" },
					Constants.TABLE_PREFIX + Constants.TABLE_SOLVER_TEST,
					new String[] { "TestID" }, " ON DELETE CASCADE") }, // foreign
																		// key
			"" // sequence name
	);
...
//		// invoke user login RESTFul API
//
////String res = userLogin(Constants.CLUSTER_USERNAME, Constants.CLUSTER_PASSWD);
////return Response.status(200).entity("<html>Token:"+res+"</html>").build();
//	}
...
//return Response.status(200).entity("<html>Token:"+res+"</html>").build();
		...
		// Initialize the title of a science app under test to that of the given
		// science app.
		Constants.SCIENCE_APP_TITLE_UNDER_TEST = app_title;

		if (fileDetail == null || fileDetail.getFileName().equals("")) {
			return Response.status(200).entity("No Science App Uploaded")
					.build();
		}

		if (fileDetail2 == null || fileDetail2.getFileName().equals("")) {
			return Response.status(200)
					.entity("No Science App Input File Uploaded").build();
		}
		...
		System.out.println("science input file name: "
				+ scienceAppInputFileName);

		// Specify a directory to store an uploaded Science App file.
		String scienceAppPath = Constants.SCIENCE_APP_LOCATION
				+ scienceAppFileName;
		...
		// Specify a directory to store an uploaded Science App file.
		String scienceAppInputFilePath = Constants.SCIENCE_APP_INPUT_FILE_LOCATION
				+ scienceAppInputFileName;
		// System.out.println(uploadedFileLocation);
		...
		// System.out.println(scienceAppPath +
		// " has been granted for executable.");
		// }catch(Exception ex){
		...
		// String scienceAppOutputFilePath =
		// Constants.SCIENCE_APP_OUTPUT_FILE_LOCATION + "output.txt";
		String scienceAppOutputFileName = "output.txt";
		String value = submitJob(scienceAppFileName, scienceAppInputFileName,
				scienceAppOutputFileName);

		if (value.contains("error")) {
			return Response
					.status(200)
					.entity("<html>" + scienceAppPath
							+ " has no output, which seems problematic ("
							+ value + "</html>").build();
		} else {
			String response = "<a href=\"" + value + "\">result</a>";
			System.out.println(response);
			return Response.status(200).entity("<html>" + response + "</html>")
					.build();
		}
	}

	/***
	 * Login to a test cluster with given user name and password
	 * @param userName user name
	 * @param passwd password
	 * @return user token
	 */
	private String userLogin(String userName, String passwd) {
		// TODO Auto-generated method stub
		// Declare a http RESTful client
		Client client = ClientBuilder.newClient(new ClientConfig());
		// Declare a web target resource.
		WebTarget loginWebTarget = client.target(Constants.IB_ACCESS_PT+"/"+Constants.LOGIN_RESTAPI);
		// Print out an URI for a check.
		System.out.println("address: " + loginWebTarget.getUri().toString());
		
		// Build an XML body.
		String xmlBody = "<login><userId>" + userName 
				+ "</userId><password>"+ passwd +"</password></login>";
		System.out.println(xmlBody);
//		Response postResponse =
//				//loginWebTarget.request().header("application/xml", arg1).accept(MediaType.TEXT_PLAIN_TYPE).post(Entity.entity(xmlBody, MediaType.TEXT_XML_TYPE), Response.class);
//				loginWebTarget.request(MediaType.TEXT_PLAIN).header("application/xml", "true").post(Entity.entity(xmlBody, MediaType.TEXT_XML_TYPE), Response.class);
		// Construct an builder for invocation
		// Set a text/plain type
		// Set accept type using application/xml
		// Set content type using application/xml
		Invocation.Builder builder =
			loginWebTarget.request(MediaType.TEXT_PLAIN_TYPE)
			.accept(MediaType.APPLICATION_XML);
		// Invoke a RESTful API with POST method
		Response postResponse = builder.post(Entity.entity(xmlBody, MediaType.APPLICATION_XML));
		// Print status code
		System.out.println(postResponse.getStatus());
		// Read a response entity
		String res = postResponse.readEntity(String.class);
		System.out.println(res);
		
		String token = parseToken(res);
		return token;
	}

	/****
	 * Parse a given token from the returned content
	 * @param res
	 * @return user login token
	 */
	private String parseToken(String res) {
		// TODO Auto-generated method stub
		String token = "";
		StringTokenizer st = new StringTokenizer(res, ":| |\t|\n");
		//"Token: eWtzdWg6MTQ0NTgxOTE4MTcyNyMkJV4lJCMxNDQ1OTA1NTgxNzI3 Expired: Tue Oct 27 18:26:21 KST 2015"
		int tokenIdx = 0;
	 ...
	 * 
	 * @param sciAppFileName
	 *            science app file name
	 * @param sciAppInputFileName
	 *            science app input file name
	 * @param sciAppOutputFileName
	 *            science app output file name
	 * @return final execution result value
	 */
	public String submitJob(String sciAppFileName, String sciAppInputFileName,
		...
		String[] cmd = { "/bin/sh", "-c",
				"cd " + Constants.SCIENCE_APP_LOCATION + "; " + exec };
		...
		String line = "";
		String value = "";
		String outputFilePath = Constants.SCIENCE_APP_OUTPUT_FILE_LOCATION
				+ sciAppOutputFileName;
...
//		return Constants.SCIENCE_APP_OUTPUT_FILE_LOCATION
//				+ sciAppOutputFileName;
		return "../../../sci_apps/result/" + sciAppOutputFileName;
	}
	...
	public void connectToDB() {
		...
		mysqlDBMS = new MySQLSubject(Constants.DBMS_USERNAME,
				Constants.DBMS_PASSWORD, Constants.DBMS_CONNECT_STRING);
		// let's make a connection to MySQL DBMS.
		mysqlDBMS.open(false);
		System.out
				.println("A connection to the MySQL DBMS instance has been successfully created.");
	}
	...
	public void createTestTables() {
		...
		// Create test tables.
		// Create solver table.
		// In service code the solver table in the EDISON portal db should be
		// accessed.
		mysqlDBMS.createTable(SOLVER.TableName, SOLVER.columnNames,
				SOLVER.columnDataTypes, SOLVER.columnDataTypeLengths,
				SOLVER.autoIncrementColumns, SOLVER.columnNullable,
				SOLVER.uniqueConstraintColumns, SOLVER.primaryKey,
				SOLVER.foreignKey);

		// Create solver test table.
		// In service code the solver test table in the EDISON portal db should
		// be accessed.
		mysqlDBMS.createTable(SOLVER_TEST.TableName, SOLVER_TEST.columnNames,
				SOLVER_TEST.columnDataTypes, SOLVER_TEST.columnDataTypeLengths,
				SOLVER_TEST.autoIncrementColumns, SOLVER_TEST.columnNullable,
				SOLVER_TEST.uniqueConstraintColumns, SOLVER_TEST.primaryKey,
				SOLVER_TEST.foreignKey);
		// Create solver test log table.
		// In service code the solver test log table in the EDISON portal db
		// should be accessed.
		mysqlDBMS.createTable(SOLVER_TEST_LOG.TableName,
				SOLVER_TEST_LOG.columnNames, SOLVER_TEST_LOG.columnDataTypes,
				...
				SOLVER_TEST_LOG.primaryKey, SOLVER_TEST_LOG.foreignKey);
		mysqlDBMS.commit();
		// disconnectToDB();
	}
	 ...
	 */
	private void insertRowsIntoTables(String title, String name,
			String description) {
		// If mysql dbms is not created, then
		if (mysqlDBMS == null) {
			// report an error.
		 ...
		 */
		mysqlDBMS.buildInsertSQL(SOLVER.TableName, colNames, colValues,
				colTypes);

		String sql = "select " + SOLVER.columnNames[0] + " from "
				+ SOLVER.TableName + " order by " + SOLVER.columnNames[0];
		int solverID = -1;
		...
		if (solverID != -1) {
			 ...
			 */
			mysqlDBMS.buildInsertSQL(SOLVER_TEST.TableName, testColNames,
					testColValues, testColTypes);
		}

		// retrieve testID
		sql = "select TestID " + " from " + SOLVER_TEST.TableName
		...
		if (testID != -1) {
			 ...
			 */
			mysqlDBMS.buildInsertSQL(SOLVER_TEST_LOG.TableName, testColNames,
					testColValues, testColTypes);
		}
	...
	public void retrieveStatusForSolverInTest() {
		...
		String sql = "select t0.SolverID, t0.Title as solver_title, t1.TestStartTime, t2.TestStatus, t2.TestStatusUpdateTime "
				+ SOLVER.columnNames[0]
				+ " from "
				+ SOLVER.TableName
				+ " as t0, "
				+ SOLVER_TEST.TableName
				+ " as t1, "
				+ SOLVER_TEST_LOG.TableName
				+ " as t2 "
		...
		if (rs != null) {
			try {
				...
				while (rs.next()) {
					...
					testStartTime = new SimpleDateFormat(Constants.TIMEFORMAT)
							.format(rs.getTimestamp(3));
					testStatus = rs.getString(4);
					testStatusUpdateTime = new SimpleDateFormat(
							Constants.TIMEFORMAT).format(rs.getTimestamp(5));
					System.out.format("%d | %s | %s | %s | %s\n", solverID,
							title, testStartTime, testStatus,
							testStatusUpdateTime);
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	public void disconnectToDB() {
		...
		mysqlDBMS.close();
		System.out
				.println("We have successfully disconnected from MySQL DBMS.");
	}
	...
	@Path("/module/list_by_keyword")
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	public String retrieveModuleByKeyword(
			@FormDataParam("modName") String mod_name) {
	...
	@Path("/module/load")
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	public Response loadModuleByKeyword(
			@FormDataParam("mod_name") String mod_name) {
		// We internally invoke retrieveLibrary() and
		// then search for a library(ies) to contain a given library name
		// (libname)
	...
	// @PathParam injects the value of URI parameter that defined in @Path
	// expression, into the method.
	public Response retrieveLibraryByKeyword(
			@FormDataParam("libNameKeyword") String lib_name) {
		if (lib_name == null)
			// Return the final library list string.
			return Response.status(200).entity("<html>No keyword!</html>")
					.build();
	...
	public Response installLibrary(
		...
		// "/usr/local/apache-tomcat-7.0.63/webapps/data/" +
		// fileDetail.getFileName();
		String uploadedFileLocation = Constants.TOMCAT_LOCATION
				+ Constants.WEBAPP_LOCATION + Constants.UPLOAD_LOCATION
				+ fileDetail.getFileName();
		// System.out.println(uploadedFileLocation);
		...
		//
		// String output =
		// "File uploaded via Jersey based RESTFul Webservice to: " +
		...
		if (errorOutput.equalsIgnoreCase("")) {
			return Response
					.status(200)
					.entity("<html>Successful Installation of '"
							+ fileDetail.getFileName() + "' !" + normalOutput
							+ "</html>").build();
		}
		// If there's any error,
			...
			return Response
					.status(200)
					.entity("<html>Installation Error Occurred: " + errorOutput
							+ "</html>").build();
		}
	}
	...
	public Response retrieveLibrary() {
		...
		// The following two lines should be replaced by the third line when in
		// service.
		...
		try {
			...
			while ((line = buf_reader.readLine()) != null) {
				...
				// System.out.println(libName + "|" + linuxCLibVer + "|" +
				// sysArch
				// + "|" + kernelVer + "|" + libPath);
		...
		for (int i = 0; i < libList.size(); i++) {
			// Convert the library instance data into the corresponding XML
			// document.
